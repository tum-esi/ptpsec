...
// PTPsec
static void measurement_pre_send(struct measurement_msg *m)
{
	m->meas_type = htons(m->meas_type);
}
// end PTPsec

// PTPsec
static void measurement_post_recv(struct measurement_msg *m)
{
	m->meas_type = ntohs(m->meas_type);
}
// end PTPsec
...
int msg_post_recv(struct ptp_message *m, int cnt)
{
	...
	// PTPsec
	case MEASUREMENT:
		pdulen = sizeof(struct measurement_msg);
		break;
	// end PTPsec
	default:
		return -EBADMSG;
	...
	// PTPsec
	case MEASUREMENT:
		timestamp_post_recv(m, &m->measurement.originTimestamp);	// dependent on message type: origin, preciseOrigin, rt
		measurement_post_recv(&m->measurement);
		break;
	// end PTPsec
	}
...
int msg_pre_send(struct ptp_message *m)
{
	...
	// PTPsec
	case MEASUREMENT:
		timestamp_pre_send(&m->measurement.originTimestamp);	// dependent on message type: origin, preciseOrigin, rt
		measurement_pre_send(&m->measurement);
		break;
	// end PTPsec
	default:
		return -1;
