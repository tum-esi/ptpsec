...
// PTPsec
// Move definition of clock into header file
// Move definition of freq_estimator into header file
// Move definition of clock_stats into header file
// end PTPsec
										...
										tmv_t origin)
{
	...
	FILE *file;
	file = fopen("./sync.txt", "a");
	if (file)
	{
		int err = fprintf(file, "%16lu, %16ld, %16ld\n", (unsigned long)time(NULL), tmv_to_nanoseconds(c->master_offset), tmv_to_nanoseconds(c->path_delay));
		if (err < -1)
		{
			pr_err("Error reading sync.txt: (Err: %d)\n", err);
		}
		fclose(file);
	}
	else
	{
		pr_err("Couldn't open sync.txt\n");
	}
	// end PTPsec
						  ...
						  struct interface *iface)
{
	...
	// PTPsec
	pr_debug("port %d (%s) added to clock", phc_index, phc_device);
	// end PTPsec
						   ...
						   const char *phc_device)
{
	...
	// Select clock from first interface as PHC
	...
	// PTPsec
	// Init linked list for measurement records
	LIST_INIT(&c->measurements);
	// end PTPsec
...
int clock_poll(struct clock *c)
{
	...
	// pr_debug("     Clock Time: %ld.%ld", ts.tv_sec, ts.tv_nsec);
	clock_gettime(c->clkid_meas, &ts);
	// pr_debug("Meas Clock Time: %ld.%ld", ts.tv_sec, ts.tv_nsec);
