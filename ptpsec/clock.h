...
struct path_measurement {
	LIST_ENTRY(path_measurement) list;	// First element in linked list
	UInteger16 sequenceId;   // Packet sequence number
	tmv_t t1;   // 1st PTP timestamp (either t1 (master RT) or t3 (slave RT))
	tmv_t t2;   // 2nd PTP timestamp (either t2 (master RT) or t4 (slave RT))
	tmv_t tm1;  // 1st MEASUREMENT timestamp (either tm1 (master RT) or tm3 (slave RT))
	tmv_t tm2;  // 2nd MEASUREMENT timestamp (either tm2 (master RT) or tm4 (slave RT))
	tmv_t rt;   // Fully computed round trip time on master side (tm)
};
...
struct clock {
	...
	int step_window;
	// PTPsec
	LIST_HEAD(path_measurement_head, path_measurement) measurements;
	// end PTPsec
};
// end PTPsec
