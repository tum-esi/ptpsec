										 ...
										 struct ptp_message *rsp, int id)
{
	...
	case MID_PORT_HWCLOCK_NP:
		...
		phn->flags = interface_get_vclock(target->iface) >= 0 ? PORT_HWCLOCK_VCLOCK : 0;
		datalen = sizeof(*phn);
		break;
	default:
							 ...
							 Integer8 sync_interval)
{
	...
	// PTPsec
	tmv_t t2c = tmv_sub(t2, tmv_add(c1, c2));	// t2' = t2 - c1 - c2 (instead of correcting t1)
	broadcast_measurement(p, seqid, t2c, NULL);
	// end PTPsec
...
static int port_pdelay_request(struct port *p)
{
	...
	msg->header.logMessageInterval = port_is_ieee8021as(p) ? p->logPdelayReqInterval : 0x7f;
...
int port_delay_request(struct port *p)
{
	...
	// PTPsec
	uint32_t seqid = msg->header.sequenceId;
	// end PTPsec
	...
	// PTPsec
	tmv_t t3 = msg->hwts.ts;
	save_measurement(p->clock, p, MEAS_T1, t3, seqid);
	// end PTPsec
...
int port_tx_sync(struct port *p, struct address *dst, uint16_t sequence_id)
{
	...
	// PTPsec
	tmv_t t1 = msg->hwts.ts;
	save_measurement(p->clock, p, MEAS_T1, t1, sequence_id);
	// end PTPsec
...
static int process_delay_req(struct port *p, struct ptp_message *m)
{
	...
	// PTPsec
	broadcast_measurement(p, m->header.sequenceId, m->hwts.ts, &m->address);
	// end PTPsec
...
void process_delay_resp(struct port *p, struct ptp_message *m)
{
	...
	// PTPsec
	save_measurement(p->clock, p, MEAS_T2, t4c, rsp->hdr.sequenceId);
	// end PTPsec
...
// PTPsec
int broadcast_measurement(struct port *p, int seqenceId, tmv_t t, struct address *addr)
{
	// Create new measurement msg
	int err;
	struct ptp_message *msg, *fup;
	msg = msg_allocate();
	if (!msg)
	...
	// Send on all other ports but the current one (where the PTP message arrived)
	struct clock *c;
	c = p->clock;
	struct port *p_meas;
	LIST_FOREACH(p_meas, &c->ports, list)
	{
		// Check for detection port
		if (p_meas->state != PS_DETECTION)
		{
			continue;
		}

		// Set headers accordingly
		msg->hwts.type = p_meas->timestamping;

		msg->header.tsmt = MEASUREMENT | p_meas->transportSpecific;
		msg->header.ver = PTP_VERSION;
		msg->header.messageLength = sizeof(struct measurement_msg);
		msg->header.domainNumber = clock_domain_number(p_meas->clock);
		msg->header.correction = -p_meas->asymmetry;
		if (is_detection_master(p))
			msg->header.correction *= -1;	// positive asym in M->S direction
		msg->header.sourcePortIdentity = p_meas->portIdentity;
		msg->header.sequenceId = seqenceId;
		msg->header.control = CTL_MEASUREMENT;
		msg->header.logMessageInterval = p_meas->logSyncInterval;
		msg->header.flagField[0] |= TWO_STEP;

		msg->measurement.meas_type = MEAS_MEASUREMENT;
		msg->measurement.originTimestamp = tmv_to_Timestamp(t); // either t2 (at slave) or t4 (at master)

		// Send message
		if (port_prepare_and_send(p_meas, msg, TRANS_EVENT))
		{
			pr_err("%s: send measurement failed", p_meas->log_name);
			goto out;
		}

		if (msg_sots_missing(msg))
		{
			pr_err("missing timestamp on transmitted measurement (broadcast)");
			goto out;
		}

		pr_debug("MEAS sent from port: %d", port_number(p_meas));

		/*
		 * Send the follow up message right away.
		 */
		fup->hwts.type = p_meas->timestamping;

		fup->header.tsmt = MEASUREMENT | p_meas->transportSpecific;
		fup->header.ver = PTP_VERSION;
		fup->header.messageLength = sizeof(struct measurement_msg);
		fup->header.domainNumber = clock_domain_number(p_meas->clock);
		fup->header.sourcePortIdentity = p_meas->portIdentity;
		fup->header.sequenceId = seqenceId;
		fup->header.control = CTL_MEASUREMENT;
		fup->header.logMessageInterval = p_meas->logSyncInterval;

		fup->measurement.meas_type = MEAS_FOLLOW_UP;
		fup->measurement.originTimestamp = tmv_to_Timestamp(msg->hwts.ts);

		// Send message
		err = port_prepare_and_send(p_meas, fup, TRANS_GENERAL);
		if (err)
		{
			pr_err("%s: send follow up failed", p_meas->log_name);
			goto out;
		}

		pr_debug("MEAS FUP sent from port: %d", port_number(p_meas));
	}
...
// PTPsec
int send_measurement(struct port *p, int seqenceId, tmv_t t)
{
	// Create new measurement msg
	struct ptp_message *msg;
	msg = msg_allocate();
	...
	LIST_FOREACH(p_meas, &c->ports, list)
	{
		...
		else
		{
			...
			msg->measurement.meas_type = MEAS_TRANSPORT;
			msg->measurement.rtMeasurement = tmv_to_Timestamp(t); // Computed RT measurement

			if (port_prepare_and_send(p_meas, msg, TRANS_GENERAL))
			{
				pr_err("%s: send measurement failed", p_meas->log_name);
				goto out;
			}
			// For all other ports
			pr_debug("MEAS TRANSPORT sent from port: %d", port_number(p_meas));
		}
	}
...
// PTPsec
void process_measurement(struct port *p, struct ptp_message *m)
{
	// Get general packet infos
	uint16_t seqid = m->header.sequenceId;
	struct timestamp origin_ts = m->ts.pdu;

	// Declaring some variables (due to C syntax: https://stackoverflow.com/questions/18496282/why-do-i-get-a-label-can-only-be-part-of-a-statement-and-a-declaration-is-not-a)
	tmv_t tm1, tm1c, cm1, t2, tm2, tm2c, cm2;

	// Check whether we received MEAS, M_FUP or TRANSPORT
	switch (m->measurement.meas_type)
	{
	case MEAS_MEASUREMENT:
		// Account for port asymmetry
		m->header.correction = p->asymmetry;
		if (is_detection_master(p))
			m->header.correction *= -1;

		t2 = timestamp_to_tmv(origin_ts);
		tm2 = m->hwts.ts;
		cm1 = correction_to_tmv(m->header.correction);
		tm2c = tmv_sub(tm2, cm1);	// tm2' = tm2 - cm1

		if (is_detection_master(p))
			save_measurement(p->clock, p, MEAS_T2, t2, seqid);
		save_measurement(p->clock, p, MEAS_TM2, tm2c, seqid);
		break;

	case MEAS_FOLLOW_UP:
		// Get exact egress timestamp of event msg
		tm1 = timestamp_to_tmv(origin_ts);

		// Get corrections
		cm2 = correction_to_tmv(m->header.correction);
		tm1c = tmv_sub(tm1, cm2);

		save_measurement(p->clock, p, MEAS_TM1, tm1c, seqid);
		break;

	case MEAS_TRANSPORT:
		pr_debug("### Measurement TRANSPORT");

		// Get matching sequence number
		// seqid = p->seqnum.delayreq - 1;
		// Get timestamp from message
		tmv_t rt_master = timestamp_to_tmv(origin_ts);
		// Save to appropriate record
		save_measurement(p->clock, p, MEAS_RT, rt_master, seqid);
		break;

	default:
		pr_err("Unknown MEAS TYPE: %d", m->measurement.meas_type);
	}
}
// end PTPsec

// PTPsec
bool is_detection_master(struct port *p)
{
	// Check for detection port on master node
	bool is_master = false;
	struct port *p_check;
	struct clock *c = p->clock;
	LIST_FOREACH(p_check, &c->ports, list)
	{
		// Check for master state
		if ((p_check->state == PS_MASTER) || (p_check->state == PS_GRAND_MASTER))
		{
			is_master = true;
			break;
		}
...
// PTPsec
/*
 * ts (tmv_t): timestamp that should be added to the measurement record
 * type (enum meas_ts_type): which timestamp is it?
 * sequenceId (int): which PTP iteration
 */
int save_measurement(struct clock *c, struct port *p, enum meas_ts_type type, tmv_t ts, int sequenceId)
{
	pr_debug("Save measurement (Id: %d, Type: %d)", sequenceId, type);
	...
	// Search for existing measurement with same sequence id
	struct path_measurement *m, *tmp;
	bool existing = false;
	LIST_FOREACH_SAFE(m, &c->measurements, list, tmp)
	{
		if (m->sequenceId == sequenceId)
		{
			existing = true;
			break;
		}
	...
	if (!existing)
	{
		...
		if (!m)
		{
			pr_err("failed to allocate memory for a measurement");
			return -1;
		}
		...
		LIST_INSERT_HEAD(&c->measurements, m, list);

		// Debug output
		// pr_debug("New measurement record created (ID=%d)", sequenceId);
	}
	...
	case MEAS_RT:
		...
		break;
	default:
		pr_err("Unknown MEAS timestamp type");
		return -1;
	}

	// pr_debug("#################### Measurement (RT): ####################");
	// pr_debug("\t     ID = %d", sequenceId);
	// pr_debug("\t     t1 = %ld\n\t\t\t     t2 = %ld", m->t1.ns, m->t2.ns);
	// pr_debug("\t    tm1 = %ld\n\t\t\t    tm2 = %ld", m->tm1.ns, m->tm2.ns);
	// pr_debug("\t     rt = %ld", m->rt.ns);

	// Check whether all required measurements/timestamps are available
	if (!tmv_is_zero(m->t1) && !tmv_is_zero(m->tm1) && !tmv_is_zero(m->t2) && !tmv_is_zero(m->tm2))
	{
		// -> All timestamps are set

		// Compute final RT measurement (t2' - t1 + tm2' - tm1')
		tmv_t rt = tmv_add(tmv_sub(m->t2, m->t1), tmv_sub(m->tm2, m->tm1));

		// Either send record to slave or report asymmetry
		if (is_detection_master(p))
		{ // Master
			// Send result to slave
			send_measurement(p, sequenceId, rt);
		...
		{
			// if (!tmv_is_zero(m->rt))
			if (!tmv_is_zero(latest_master_rt))
			{
				tmv_t asym = tmv_sub(latest_master_rt, rt);
				// tmv_t delay = tmv_div(asym, 2);

				pr_info("\t  asym = %7ld\n", asym.ns);

				// Save to file for logging
				FILE *f;
				f = fopen("./measurements.txt", "a");
				if (f)
				{
					int err = fprintf(f, "%16lu, %16ld\n", (unsigned long)time(NULL), tmv_to_nanoseconds(asym));
					if (err < -1)
					{
						pr_err("Error reading measurements.txt: (Err: %d)\n", err);
					}
					fclose(f);
				}
				else
				{
					pr_err("Couldn't open measurements.txt\n");
				}

				// Remove entry from list
				pr_debug("Measurement record deleted (ID=%d)", sequenceId);
				LIST_REMOVE(m, list);
			}
			else
			{
				pr_debug("Latest Master RTT is zero");
			}
		}
	}
...
// PTPsec
static void detection_dispatch(struct port *p, enum fsm_event event, int mdiff)
{
	if (clock_slave_only(p->clock))
	{
		if (event == EV_RS_GRAND_MASTER)
...
static enum fsm_event bc_event(struct port *p, int fd_index)
{
	...
	case FD_MANNO_TIMER:
		...
		return port_tx_announce(p, NULL, p->seqnum.announce++) ? EV_FAULT_DETECTED : EV_NONE;

	case FD_SYNC_TX_TIMER:
		pr_debug("%s: master sync timeout", p->log_name);
		port_set_sync_tx_tmo(p);
		p->service_stats.master_sync_timeout++;
		return port_tx_sync(p, NULL, p->seqnum.sync++) ? EV_FAULT_DETECTED : EV_NONE;
...
// PTPsec
static enum fsm_event detection_event(struct port *p, int fd_index)
{
	enum fsm_event event = EV_NONE;
	struct ptp_message *msg;
	int cnt, fd = p->fda.fd[fd_index], err;
	...
	// No timer expired but message received
	msg = msg_allocate();
	if (!msg)
		return EV_FAULT_DETECTED;
	...
	{
	case SYNC:
		pr_debug("### PTPsec: Sync (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		process_sync(p, msg);
		break;
	case DELAY_REQ:
		pr_debug("### PTPsec: Delay Req (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		if (process_delay_req(p, msg))
			event = EV_FAULT_DETECTED;
		break;
	case PDELAY_REQ:
		pr_debug("### PTPsec: PDelay Req (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		if (process_pdelay_req(p, msg))
			event = EV_FAULT_DETECTED;
		break;
	case PDELAY_RESP:
		pr_debug("### PTPsec: PDelay Resp (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		if (process_pdelay_resp(p, msg))
			event = EV_FAULT_DETECTED;
		break;
	case FOLLOW_UP:
		pr_debug("### PTPsec: Follow Up (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		process_follow_up(p, msg);
		break;
	case DELAY_RESP:
		pr_debug("### PTPsec: Delay Resp (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		process_delay_resp(p, msg);
		break;
	case PDELAY_RESP_FOLLOW_UP:
		pr_debug("### PTPsec: PDelay Resp Follow Up (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		process_pdelay_resp_fup(p, msg);
		break;
	case ANNOUNCE:
		pr_debug("### PTPsec: Announce received on %s", port_log_name(p));
		if (process_announce(p, msg))
			event = EV_STATE_DECISION_EVENT;
		break;
	case SIGNALING:
		pr_debug("### PTPsec: Signaling received on %s", port_log_name(p));
		if (process_signaling(p, msg))
		{
			event = EV_FAULT_DETECTED;
		}
		break;
	case MANAGEMENT:
		pr_debug("### PTPsec: Management received on %s", port_log_name(p));
		if (clock_manage(p->clock, p, msg))
			event = EV_STATE_DECISION_EVENT;
		break;
	case MEASUREMENT:
		pr_debug("### PTPsec: Measurement (Id: %d) received on %s", msg->header.sequenceId, port_log_name(p));
		process_measurement(p, msg);
		break;
	}
					   ...
					   struct clock *clock)
{
	...
	case CLOCK_TYPE_E2E:
		...
		break;
	// PTPsec
	case CLOCK_TYPE_DETECTION:
		p->dispatch = detection_dispatch;
		p->event = detection_event;
		break;
	// end PTPsec
...
int port_state_update(struct port *p, enum fsm_event event, int mdiff)
{
	...
	if (PS_INITIALIZING == next)
	{
		...
		else
		{
			...
			if(port_number(p) > 1)
			{
				// Put detection port in dedicated state
				pr_debug("   > Port %d: Detection init complete!", port_number(p));
				event = EV_DETECT_INIT_COMPLETE;
			}
			else
			{
				pr_debug("   > Port %d: Default init complete!", port_number(p));
			// end PTPsec
