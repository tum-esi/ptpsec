...
// PTPsec
// Variables for HWTS feature ()
extern uint64_t igb_timestamp_dynflag;
extern int igb_timestamp_dynfield_offset;
// end PTPsec
				  ...
				  uint64_t ol_flags, union igb_tx_offload tx_offload)
{
	...
	{ /* no TSO, check if hardware checksum is needed */
...
rx_desc_status_to_pkt_flags(uint32_t rx_status)
{
	...
	pkt_flags = ((rx_status & E1000_RXD_STAT_VP) ? RTE_MBUF_F_RX_VLAN | RTE_MBUF_F_RX_VLAN_STRIPPED : 0);
				  ...
				  uint16_t nb_pkts)
{
	...
	while (nb_rx < nb_pkts)
	{
		...
		// PTPsec
		/*
		 * Handle HWTS offloading if enabled
		 */
		if (igb_timestamp_dynflag > 0 && (rxq->offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP))
		{
			// TSIP bit in the RDESC.STATUS
			uint32_t tsip = rte_le_to_cpu_16(rxd.wb.upper.status_error) & (0x1 << 15);
			if(tsip)
			{
				// Remove HWTS from buffer
#define TIMESTAMP_BUFFER_SIZE 16
				pkt_len = (uint16_t)(rte_le_to_cpu_16(rxd.wb.upper.length) -
									rxq->crc_len - TIMESTAMP_BUFFER_SIZE);
				rxm->data_off = RTE_PKTMBUF_HEADROOM + TIMESTAMP_BUFFER_SIZE;

				// Read provided HWTS from buffer
				char *p = (char *) rxm->buf_addr + rxm->data_off - TIMESTAMP_BUFFER_SIZE;
				p+=8;
				uint32_t systiml = rte_le_to_cpu_32(*((uint32_t*) p)); p+=4;
				uint32_t systimh = rte_le_to_cpu_32(*((uint32_t*) p)); p+=4;
				uint64_t ts_ns = (systimh * NSEC_PER_SEC) + systiml;

				// Set HWTS in related mbuf field 
				*RTE_MBUF_DYNFIELD(rxm,
								igb_timestamp_dynfield_offset,
								rte_mbuf_timestamp_t *) = ts_ns;
				// Indicate HWTS in offload flags
				rxm->ol_flags |= igb_timestamp_dynflag;
...
int eth_igb_tx_queue_setup(struct rte_eth_dev *dev,
						   uint16_t queue_idx,
						   uint16_t nb_desc,
						   ...
						   const struct rte_eth_txconf *tx_conf)
{
	...
	txq->reg_idx = (uint16_t)((RTE_ETH_DEV_SRIOV(dev).active == 0) ? queue_idx : RTE_ETH_DEV_SRIOV(dev).def_pool_q_idx + queue_idx);
...
int eth_igb_rx_queue_setup(struct rte_eth_dev *dev,
						   uint16_t queue_idx,
						   uint16_t nb_desc,
						   unsigned int socket_id,
							...
							struct rte_eth_rss_conf *rss_conf)
{
	...
	mrqc = E1000_READ_REG(hw, E1000_MRQC);
	if (!(mrqc & E1000_MRQC_ENABLE_MASK))
	{					 /* RSS disabled */
...
igb_vmdq_rx_hw_configure(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < cfg->nb_pool_maps; i++)
	{
		...
		E1000_WRITE_REG(hw, E1000_VLVF(i), (E1000_VLVF_VLANID_ENABLE | (cfg->pool_map[i].vlan_id & RTE_ETH_VLAN_ID_MAX) | ((cfg->pool_map[i].pools << E1000_VLVF_POOLSEL_SHIFT) & E1000_VLVF_POOLSEL_MASK)));
	}
...
int eth_igb_rx_init(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < dev->data->nb_rx_queues; i++)
	{
		...
		// PTPsec
		// Enable to place HWTS at the beginning of a receive buffer for each packet
		srrctl |= E1000_SRRCTL_TIMESTAMP;

		// Indicate required buffer size for additional timestamps
#define E1000_RXPBSIZE 0x2404
#define E1000_RXPBSIZE_CFG_TS_EN (0x1 << 31)
		uint32_t rxpbsize = E1000_READ_REG(hw, E1000_RXPBSIZE);
		rxpbsize |= E1000_RXPBSIZE_CFG_TS_EN;
		E1000_WRITE_REG(hw, E1000_RXPBSIZE, rxpbsize);
		// end PTPsec
