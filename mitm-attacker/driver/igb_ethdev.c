...
// PTPsec
uint64_t igb_timestamp_dynflag;
int igb_timestamp_dynfield_offset = -1;
// end PTPsec
...
// 				  struct timespec *timestamp);
static int igb_timesync_read_time(struct rte_eth_dev *dev,
				  struct timespec *timestamp, int64_t *correction);
// PTPsec
static int igb_timesync_adj_inc_val(struct rte_eth_dev *dev, uint32_t inc_val);
static int igb_timesync_adj_val(struct rte_eth_dev *dev, int64_t delta);
static int igb_timesync_get_offset_to_pps(struct rte_eth_dev *dev, int64_t *offset, uint64_t *clock_nsec);
// end PTPsec
...
static const struct eth_dev_ops eth_igb_ops = {
	...
	// PTPsec
	.timesync_adj_inc_val = igb_timesync_adj_inc_val,
	.timesync_adj_val     = igb_timesync_adj_val,
	.timesync_get_offset_to_pps = igb_timesync_get_offset_to_pps,
	// end PTPsec
...
eth_igb_start(struct rte_eth_dev *dev)
{
	...
	if (dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_TIMESTAMP) {
		if (hw->mac.type == e1000_i210)
		{
			...
			ret = rte_mbuf_dyn_rx_timestamp_register(&igb_timestamp_dynfield_offset,
								&igb_timestamp_dynflag);
			if (ret) {
...
eth_igb_infos_get(struct rte_eth_dev *dev, struct rte_eth_dev_info *dev_info)
{
	...
	if (hw->mac.type == e1000_i210)
	{
		// Add RX HWTS offloading capabilities
...
igb_timesync_adjust_time(struct rte_eth_dev *dev, int64_t delta)
{
	...
	systime_cycles += delta;

	E1000_WRITE_REG(hw, E1000_SYSTIMR, 0x0);
	E1000_WRITE_REG(hw, E1000_SYSTIML, systime_cycles & 0x3FFFFFFF);
	E1000_WRITE_REG(hw, E1000_SYSTIMH, (systime_cycles >> 32) & 0x3B9AC9FF);
	// end PTPsec
...
static int
// PTPsec
igb_timesync_read_time(struct rte_eth_dev *dev, struct timespec *ts, int64_t *correction)
// end PTPsec
// igb_timesync_read_time(struct rte_eth_dev *dev, struct timespec *ts)
{
	...
	*ts = rte_ns_to_timespec(ns);
	...
	// PTPsec
	int64_t diff = (ns - systime_cycles);
	*correction = diff;
	*ts = rte_ns_to_timespec(systime_cycles);
	// end PTPsec
...
// PTPsec
static int
igb_timesync_adj_inc_val(struct rte_eth_dev *dev, uint32_t inc_val)
{
	struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);

	#define i210_TIMINCA 0xB608		// sign (1bit) + value (31bit) * 2^(-32)ns
	E1000_WRITE_REG(hw, i210_TIMINCA, inc_val);

	return 0;
}

static int
igb_timesync_adj_val(struct rte_eth_dev *dev, int64_t delta)	// detla in [ns]
{
	struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);

	#define i210_TIMADJ 0xB60C		// sign (1bit) + zero (1bit) + adj value in ns (30bit)

	// Prepare TIMADJ register value
	uint32_t adj_val;
	uint32_t sign = 0;	// positive
	if (delta < 0)
		sign = 1; // negative
	adj_val = delta;       		// ns
	adj_val &= 0x3FFFFFFF;		// Clear sign bit
	adj_val |= (sign << 31);	// Set sign bit

	// Update register
	E1000_WRITE_REG(hw, i210_TIMADJ, adj_val);

	/* Update internal nanosecond count */
	struct e1000_adapter *adapter = dev->data->dev_private;
	adapter->systime_tc.nsec += delta;
	adapter->rx_tstamp_tc.nsec += delta;
	adapter->tx_tstamp_tc.nsec += delta;
...
static int
igb_timesync_get_offset_to_pps(struct rte_eth_dev *dev, int64_t *offset, uint64_t *clock_nsec)
{
	#define i210_AUXSTMPL0	0xB65C		// 0xB65C ; RO
	#define i210_AUXSTMPH0	0xB660		// 0xB660 ; RO
	#define NSEC_TO_SEC	1000000000L

	struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint64_t pps_time;

	// Get latched PPS trigger timestamp
	pps_time = E1000_READ_REG(hw, i210_AUXSTMPL0);
	pps_time += E1000_READ_REG(hw, i210_AUXSTMPH0) * NSEC_TO_SEC;

	// Save clock val
	*clock_nsec = pps_time;

	// We don't care about full seconds
	*offset = pps_time % NSEC_TO_SEC;
	
	// We have two trigger points (rising and falling edge)
	if (*offset > 500000000)
		*offset = *offset - 500000000;
		// *offset = (*offset + NSEC_TO_SEC) % NSEC_TO_SEC;

	// If pps_time > 0.25s, assume negative shift
	if (*offset > 250000000)
		*offset = *offset - 500000000;
...
igb_timesync_enable(struct rte_eth_dev *dev)
{
	...
	// PTPsec
	/* Enable timestamping for all PTP messages */
	tsync_ctl = E1000_READ_REG(hw, E1000_TSYNCRXCTL);
	tsync_ctl &= ~E1000_TSYNCRXCTL_TYPE_MASK;			// Clear bits
	// tsync_ctl |= E1000_TSYNCRXCTL_TYPE_ALL;				// Set filter
	tsync_ctl |= E1000_TSYNCRXCTL_TYPE_EVENT_V2;		
	// tsync_ctl |= E1000_TSYNCRXCTL_TYPE_L2_L4_V2;		
	E1000_WRITE_REG(hw, E1000_TSYNCRXCTL, tsync_ctl);

	uint32_t tsync_cfg;
	tsync_cfg = E1000_READ_REG(hw, E1000_TSYNCRXCFG);
	tsync_cfg &= ~E1000_TSYNCRXCFG_PTP_V1_CTRLT_MASK;			// Clear bits
	tsync_cfg &= ~E1000_TSYNCRXCFG_PTP_V2_MSGID_MASK;			// Clear bits
	E1000_WRITE_REG(hw, E1000_TSYNCRXCFG, tsync_cfg);

	/* Enable PPS signal output on SDP 0 (i210 data sheet: 7.8.3.3.3) */

	// i210 register values
	#define i210_TSSDP						0x003C		/* 0x003C ; R/W */
	#define i210_TSSDP_TS_SDP0_SEL			0x000000C0	/* Bits 7:6 */
	#define i210_TSSDP_TS_SDP0_SEL_FREQ0	0x00000080	/* Bits 7:6 = 0x10 */
	#define i210_TSSDP_TS_SDP0_EN			0x00000100	/* Bits 8 */

	#define i210_TSSDP_AUX0_SDP0_SEL		0x00 			/* Bits 1:0 */
	#define i210_TSSDP_AUX0_SDP1_SEL		0x01 			/* Bits 1:0 */
	#define i210_TSSDP_AUX0_TS_SDP_EN		(0x1 << 2)		/* Bits 2 */

	#define i210_CTRL	0x00000		/* 0x00000; R/W */
	#define i210_CTRL_SDP0_IODIR 	0x00400000	/* Bits 22 (0 input, 1 output) */
	#define i210_CTRL_SDP1_IODIR 	0x00800000	/* Bits 23 (0 input, 1 output) */

	#define i210_FREQOUT0	0xB654	/* 0xB654; R/W */
	#define i210_FREQOUT1	0xB658	/* 0xB658; R/W */

	#define i210_TRGTTIML0	0xB644	/* 0xB644; R/W */
	#define i210_TRGTTIMH0	0xB648	/* 0xB648; R/W */
	#define i210_TRGTTIML1	0xB64C	/* 0xB64C; R/W */
	#define i210_TRGTTIMH1	0xB650	/* 0xB650; R/W */

	#define i210_TSAUXC				0xB640		/* 0xB640; R/W  */
	#define i210_TSAUXC_EN_TT0		(0x1 << 0)	/* Bits 0 */
	#define i210_TSAUXC_EN_TT1		(0x1 << 1)	/* Bits 1 */
	#define i210_TSAUXC_EN_CLK0		(0x1 << 2)	/* Bits 2 */
	#define i210_TSAUXC_EN_TS0		(0x1 << 8)	/* Bits 8*/
	#define i210_TSAUXC_PLSG		(0x1 << 17)	/* Bits 17 */

	// 1. Select SDP pin
	uint32_t tssdp;
	tssdp = E1000_READ_REG(hw, i210_TSSDP);
	tssdp |= i210_TSSDP_TS_SDP0_EN;		// Set to 0x1
	E1000_WRITE_REG(hw, i210_TSSDP, tssdp);

	// 2. Select target time register that defines beginning of output pulse
	tssdp = E1000_READ_REG(hw, i210_TSSDP);
	tssdp |= i210_TSSDP_TS_SDP0_SEL_FREQ0;	// Set to 0x10 (TRGTTIML/H0)
	E1000_WRITE_REG(hw, i210_TSSDP, tssdp);

	// 3. Program the clock half cycle
	uint32_t freqout0;
	freqout0 = E1000_READ_REG(hw, i210_FREQOUT0);
	freqout0 = 0x1DCD6500;	// 500ms
	E1000_WRITE_REG(hw, i210_FREQOUT0, freqout0);

	// 4. Define SDP pin as output
	uint32_t ctrl;
	ctrl = E1000_READ_REG(hw, i210_CTRL);
	ctrl |= i210_CTRL_SDP0_IODIR;		// Set to 0x1
	E1000_WRITE_REG(hw, i210_CTRL, ctrl);

	// 5. Set the required start time
	uint32_t trgttiml0;
	trgttiml0 = E1000_READ_REG(hw, i210_TRGTTIML0);
	trgttiml0 = 0x0;	// start at 0ns
	E1000_WRITE_REG(hw, i210_TRGTTIML0, trgttiml0);

	// 6. Enable frequency clock
	// uint32_t tsauxc;
	tsauxc = E1000_READ_REG(hw, i210_TSAUXC);
	tsauxc |= i210_TSAUXC_EN_CLK0;	// Set to 0x1
	E1000_WRITE_REG(hw, i210_TSAUXC, tsauxc);


	/* Set SDP 1 as input (e.g., for sync to PPS) */

	// 1. Set SDP1 pin as input
	ctrl = E1000_READ_REG(hw, i210_CTRL);
	ctrl &= ~i210_CTRL_SDP1_IODIR;		// Set to 0x0 (Input)
	E1000_WRITE_REG(hw, i210_CTRL, ctrl);

	// 2. Select SDP pin and AUX register
	// uint32_t tssdp;
	tssdp = E1000_READ_REG(hw, i210_TSSDP);
	tssdp |= i210_TSSDP_AUX0_SDP1_SEL;			// Set to 0x01 (Select SDP1)
	tssdp |= i210_TSSDP_AUX0_TS_SDP_EN;			// Set to 0x1 (Enable SDP as external trigger)
	E1000_WRITE_REG(hw, i210_TSSDP, tssdp);

	// 3. Enable external timestamping
	// uint32_t tsauxc;
	tsauxc = E1000_READ_REG(hw, i210_TSAUXC);
	tsauxc |= i210_TSAUXC_EN_TS0;	// Set to 0x1
	E1000_WRITE_REG(hw, i210_TSAUXC, tsauxc);
	// end PTPsec
			       ...
			       uint32_t flags __rte_unused)
{
	...
	*timestamp = rte_ns_to_timespec(ns);
	...
	// PTPsec
	*timestamp = rte_ns_to_timespec(rx_tstamp_cycles);
	// end PTPsec
			       ...
			       struct timespec *timestamp)
{
	...
	*timestamp = rte_ns_to_timespec(ns);
	...
	// PTPsec
	*timestamp = rte_ns_to_timespec(tx_tstamp_cycles);
	// end PTPsec
